...
-- Another solution:
https://youtu.be/2Zne9-JyiTE?t=4160
...
CREATE PROCEDURE usp_get_holders_with_balance_higher_than (money DOUBLE)
BEGIN
    SELECT e.first_name, e.last_name
    FROM account_holders AS e
    JOIN accounts a
...
BEGIN
    ...
    FROM accounts a
    WHERE
    a.id = from_account_id) IS NULL
    OR
    (SELECT a.balance
    FROM accounts a
    WHERE
    a.id = to_account_id) IS NULL 
    OR (from_account_id =  to_account_id)
    OR amount <= 0 
    OR (SELECT a.balance
    FROM accounts a
    WHERE
    a.id = from_account_id) - amount < 0)
    THEN  ROLLBACK;
